...
def build_advisor_tools(
    ...
    """闭包捕获 FAISS 实例与 BM25 实例，构建 Advisor 工具列表。

    依赖在构建时注入闭包，不再使用模块级全局变量，避免跨请求状态泄漏；
    工具描述保持不变，后续统一注册进 ToolRegistry 供节点按组取用。
    ...
    def search_column_candidates(question: str, table: str = "", k: int = None) -> list[dict]:
        ...
        if table and column_vector_store is not None:
            table_columns = column_vector_store.columns_in_table(table)
            if not table_columns:
                return []
            # fetch_k 必须覆盖该表全部字段，否则目标字段可能因相似度排名靠后而被过滤掉
            fetch_k = max(top_k * 5, len(table_columns) * 2, 50)
            docs_with_scores = column_vector_store.similarity_search_with_score(
                question,
                k=top_k,
                filter={"table": table},
                fetch_k=fetch_k,
        ...
        # 使用混合检索
        if hybrid_retriever:
            docs_with_scores = hybrid_retriever._search(
                query=question,
                k=top_k,
                vector_store_key="column",
            )
        elif column_vector_store:
            docs_with_scores = column_vector_store.similarity_search_with_score(
                question,
                k=top_k,
            )
    ...
    def search_databases(question: str) -> str:
        ...
        # 使用混合检索
        if hybrid_retriever:
            docs = hybrid_retriever.search_databases(question, k=SEARCH_DB_K)
        elif db_vector_store:
            docs = db_vector_store.similarity_search(question, k=SEARCH_DB_K)
        else:
            docs = []
    ...
    def search_tables(question: str, database: str = "") -> str:
        ...
        # 使用混合检索
        if hybrid_retriever:
            docs = hybrid_retriever.search_tables(question, k=SEARCH_TABLE_K)
        elif table_vector_store:
            if database:
                docs = table_vector_store.similarity_search(
                    question,
                    k=SEARCH_TABLE_K,
