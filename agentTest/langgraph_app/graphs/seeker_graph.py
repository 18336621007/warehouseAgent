...
from agentTest.langgraph_app.nodes.query_error_fallback_node import query_error_fallback_node
...
# 构建执行链子图（原 Seeker）：处理明确需求的确定性任务流水线
# A1：不再产出 final_answer（回答统一由 Planner respond 撰写），
#     执行成功→persist_result 落盘后回 Planner；校验/执行失败重试耗尽→query_error_end 直接告知
...
def build_seeker_subgraph(runtime):
    ...
    graph.add_node("query_error_end", query_error_fallback_node)
    ...
    graph.add_conditional_edges(
        ...
        route_after_sql_validation,
        {
            "execute": "execute_sql",
            "fix": "prepare_sql_fix",
            "error_end": "query_error_end",
        }
    )

    graph.add_edge("prepare_sql_fix", "generate_sql")

    # SQL 执行失败重试回路：成功→persist_result（落盘回 Planner），失败→fix→generate_sql，耗尽→query_error_end
    graph.add_conditional_edges(
        "execute_sql",
        route_after_sql_execution,
        {
            ...
            "degrade": "query_error_end",
        }
    )
    graph.add_edge("prepare_sql_exec_fix", "generate_sql")

    graph.add_edge("persist_result", END)
    graph.add_edge("query_error_end", END)
