...
【user_selection规则】
结合【待澄清候选】【已确认口径】【候选口径】和对话历史判断用户是否完成了选择：
- 用户回复编号、中文序号、字段名、中文含义或口语指代（如"净增那个""我要第二个"）时，判断其指向哪个候选或已确认字段，输出对应 field；clarification_id 可省略。
- 用户上一轮已选择后继续改选（如"改成1""不要4了""换另一个"）时，以最新选择为准输出对应 field；"第一个/第二个"等指代优先参考【待澄清候选】中的候选，若候选已关闭则参考【历史展示候选（改选时参考）】与对话历史中 Advisor 上轮展示的编号顺序。
