...
所有子图运行在同一份 `AgentState`（父图合并态 = `PlannerState` + `AdvisorState` + `SeekerState`）上，使用同一个 checkpoint（`thread_id = conversation_id:topic_id`），同名字段自动透传；各智能体只读写**自己子 State 的独有字段 + 共享 `BaseState` 字段**，下文每个读/写点都标注了归属的子 State。
...
>
> 展示分层（2026-08-10）：澄清选项、口径区别说明与方案确认统一优先展示字段“原始备注”（可信）；仅当字段无原始备注时才回退 LLM 增强别名，并显式标注“系统推断别名：”，避免增强元数据误导用户。
...
- 读入 State（AdvisorState + 共享 BaseState，父图 `AgentState`）：
  ...
  ```
- 给模型的输入：`ADVISOR_SYSTEM_PROMPT` + 本轮注入上下文（`graphs/advisor_graph.py`）+ 程序已确认口径：
...
【程序已确认指标口径】- 新增回流订单数 -> ads_trip.xxx.really_add_order
...
- **检索备注**：
  - 【检索】Advisor 基于【用户需求 + `【Planner候选表】`】通过 `search_columns` 工具在 `column_vector_store`（FAISS 字段索引，`filter={"table": ...}`）定向检索字段，核验字段真实性。
  - 【变化】`【程序已确认指标口径】` 来自 `MetricClarificationService.build_resolution_context`，确保 Advisor 直接使用 `really_add_order` 而不是重新猜测。
...
flowchart TD
    ...
    F --> G["注入 Advisor 上下文 + 程序收敛 measures"]
    G --> H["写入 QueryPlan.concept_resolutions 审计"]
    H --> I["Advisor 锁定 locked 方案，等待用户最终确认"]
```
...
| `agentTest/langgraph_app/services/metric_clarification_service.py` | 候选固化、模型选择白名单校验、pending 生命周期、澄清文本、上下文注入 |
