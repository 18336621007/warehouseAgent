...
> 补充：若用户本轮不是选择而是询问“这两个口径有什么区别”，Planner 结合对话历史自行判断（不再输出 `follow_up_mode` 结构化字段），Advisor 直接基于候选的原始备注/别名/来源表生成口径区别说明，不重复展示“请选择”选项。
>
