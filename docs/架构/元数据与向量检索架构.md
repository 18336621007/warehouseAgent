...
> 现状：Planner/Advisor 使用库、表、字段三层检索；Seeker 由 confirmed_plan 驱动精确 Schema 加载；元数据支持 schema 指纹增量采集、API 结构化增强与向量库按唯一键增量同步
> [返回文档索引](../文档索引.md)
...
- 字段指纹：`sha1(字段名 | 类型 | 原始注释)`。
- 表指纹：`sha1(全部字段指纹的 JSON 映射 + 表注释)`。
- 指纹只依赖 Hive 可观测的物理元数据（名/类型/注释），不依赖 LLM 增强结果，保证"只有 Hive 变了才触发重增强"。
...
对比上次 `column_fingerprints` 与本次计算结果，得到三组字段：

- `added`：本次新增的字段。
- `modified`：名/类型/注释发生变化的字段。
- `removed`：已从 Hive 消失的字段。
...
```text
build_enriched_metadata()  # 10.1 指纹增量采集 + 增强，写 MySQL
        ↓
run_build_all()            # 10.2 按唯一键同步 FAISS（新增/更新/删除）
```

支持 `--force`（向量强制重建）、`--skip-vector`（只更新 MySQL）、`--dry-run`。
...
| `table_retired` | 废弃表清理（`auto_retire_missing=True`） |
...
| python agentTest/scripts/refresh_enum_samples.py --refresh | 枚举刷新：内容变化才写库 + 同步 column 向量层 |
...
- `scripts/sync_metadata.py`：一键"采集 + 增强 + 向量同步"。
- `scripts/refresh_enum_samples.py`：`--refresh` 内容变化才写库 + 同步向量。
