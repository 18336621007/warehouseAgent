...
> 现状：元数据检索由 Planner 工具（search_databases / search_tables / search_columns）自主调用；Seeker 由查询方案驱动精确 Schema 加载；元数据支持 schema 指纹增量采集、API 结构化增强与向量库按唯一键增量同步。（2026-09-17：Advisor 库/表/字段三层检索与 submit_query_plan 门禁已随 M4 单 Agent 改造删除）
> [返回文档索引](../文档索引.md)
...
语义层候选不再程序强制注入，而是封装为 **`search_semantic` 工具**由 Planner ReAct **自主调用**（优先级高于 RAG）；命中指标 id 由 LLM 声明、程序反查 provider 拿完整口径。分层判定如下：

```text
① Planner ReAct 自主调用 search_semantic（业务词 → 指标 → 来源表/表达式）
   ├─ 命中且唯一（1 个 或 top1-top2 gap≥SEMANTIC_UNIQUE_GAP_THRESHOLD，且本轮调用了工具）
   │    → 以语义层为准：build_plan_from_semantic 确定性构建，跳过双路召回（Planner）、
   │      跳过候选澄清（Advisor），候选直接用 source_model + expression
   ├─ 命中但歧义（多候选、gap 小）
   │    → 语义层候选注入共享状态作权威参考，Advisor 澄清；双路召回用于核验/发现
   ├─ 未调用/未命中 → 双路召回兜底（search_tables/search_columns，向量 + BM25 发现物理表/字段）
   └─ 追问落盘结果（"统计各原因多少条"等）→ 用 query_stored_result 读落盘 CSV，可不调语义层
```

职责分工：

- **语义层**：权威口径（来源表/表达式/别名），减少 LLM 猜表猜字段、减少口径漂移，可审计；
- **双路召回**：物理发现（有哪些表/字段、相似度、备注），覆盖语义层未定义的大量表/字段；
- **程序硬校验**（白名单/字段归属/`lock_query_plan`）是最终防线，语义层和召回都不替代它。
...
- 同一业务主题下存在多个真实口径。
- 多个字段名称和描述天然接近。
- 用户已经选择了其中一个字段，但其他未选字段仍然具有较高向量相似度。

因此，高相似候选数量继续保留在 Planner 日志中，用于观察召回质量、分析索引和调整 Top-K，但不再作为把 `completeness=full` 强制改成 `partial` 的硬路由规则。

> 📌 注（2026-09-17）：以下"最终门禁"为旧 Advisor 架构（`advisor_graph.py`/`submit_query_plan` 已删除）；当前由 Planner 在工具循环内结合语义层候选与完整对话自行判断，不再有独立的 Advisor 门禁节点。
...
#### Hive 连接/查询超时（`db/hive_config.py`）

PyHive 无原生超时，底层 thrift socket 可能无限挂起。新增 `apply_thrift_socket_timeout(conn, timeout_seconds)`：

- 递归穿透 SASL/Buffered 包装找到最底层 `TSocket` 设置 `setTimeout`；
- 默认 30s，可用环境变量 `HIVE_TIMEOUT_SECONDS` 覆盖；
- `HiveDataSource.query` 透传 `timeout_seconds`，SQLQueryTool 默认 180s（独立聚合多表场景 300s）。

#### 检索层精确匹配（修复 fetch_k 截断）

FAISS `filter` 语义是"先取 `fetch_k` 再过滤"，当目标表字段被高分文档挤出前 k 时，目标表字段可能召回为空。全链路改为**精确 docstore 匹配**：

- `WhitelistFilteredVectorStore.columns_in_table / documents_by_table`：直接按表名精确取该表字段文档，不依赖相似度截断；
- `TableCoverageAnalyzer`：对目标表字段做精确存在性校验；
- `search_column_candidates`：按表作用域召回，避免跨表相似噪声挤掉目标表字段。
