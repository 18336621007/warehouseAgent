...
> 最后更新：2026-08-04 | Planner/Advisor 使用库、表、字段三层检索；Seeker 已改为 confirmed_plan 驱动的精确 Schema 加载
> [返回文档索引](../文档索引.md)
...
### 问题
新增表或字段后，`build_indexes` 默认从磁盘缓存加载，不会识别新数据。使用 `--force` 则全量重建所有索引。

### 方案
对比源文档与 FAISS 中已有文档的唯一键（column 层 `table.column`，table 层 `table`，db 层 `database`），仅对新文档做 embedding 并追加。

### 使用方式

| 命令 | 行为 |
|---|---|
| python -m agentTest.scripts.build_indexes | 增量：加载缓存，识别新表字段，只追加新文档 |
| python -m agentTest.scripts.build_indexes --force | 全量：删除缓存，从头构建所有索引 |

### 实现文件
- `vectorstores/schema_vector_store.py`：`_existing_keys()` + `_doc_key()` + `load_or_build(force_rebuild=)`
- `app_builder.py`：四个 builder 函数透传 `force_rebuild` 参数
- `scripts/build_indexes.py`：传递 `args.force` 给 builder
...
## 2026-08-05 连续问答与检索边界

第13课的连续问答不会通过重新执行向量检索来解释“第二个”或“第一名”：
