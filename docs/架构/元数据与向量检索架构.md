...
Planner/Advisor 在进入双路召回（向量/BM25）前，先做**语义层匹配**（`semantic_layer` 权威口径：业务词 → 指标 → 来源表/表达式）。分层判定如下：

```text
① 语义层匹配指标（match_metrics_from_query）
   ├─ 命中且唯一（1 个 或 top1-top2 gap≥SEMANTIC_UNIQUE_GAP_THRESHOLD）
   │    → 以语义层为准：跳过【分层元数据检索结果】（Planner）、跳过候选澄清（Advisor），
   │      候选直接用 source_model + expression
   ├─ 命中但歧义（多候选、gap 小）
   │    → 语义层候选注入 prompt 作权威参考，同时保留双路召回结果核验/发现，Advisor 澄清
   └─ 未命中 → 双路召回兜底（向量 + BM25 发现物理表/字段）
```
