...
### 问题
新增表或字段后，uild_indexes 默认从磁盘缓存加载，不会识别新数据。使用 --force 则全量重建所有索引。

### 方案
对比源文档与 FAISS 中已有文档的唯一键（column 层 	able.column，table 层 	able，db 层 database），仅对新文档做 embedding 并追加。
...
### 实现文件
- ectorstores/schema_vector_store.py：_existing_keys() + _doc_key() + load_or_build(force_rebuild=)
- pp_builder.py：四个 builder 函数透传 orce_rebuild 参数
- scripts/build_indexes.py：传递 rgs.force 给 builder
